...
def train_loop(dataloader, model, loss_fn, optimizer, batch_size, datakind = "bfi", device = None, v = False, batch_no_print = 100):
    ...
    for batch, data in enumerate(dataloader):
        if datakind == "bfi":
            ...
            y_domain = torch.Tensor(data["domain"]).long()
        else:
            X = data[0]
            if len(data[1]) != len(X):
                y_label = data[1][0]
        ...
        for batch, data in enumerate(dataloader):
            ...
            if isinstance(batch_no_print, int):
                if batch % batch_no_print == 0 and batch != 0:
                    test_loss /= batch
                    correct_label  /= len(dataloader.dataset) if no_batches is None else batch*dataloader.batch_size
                    correct_domain /= len(dataloader) if no_batches is None else batch
